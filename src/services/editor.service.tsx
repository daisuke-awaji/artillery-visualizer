...
export class EditorService {
  ...
  static getValue() {
    return this.getInstance()?.getModel()?.getValue() as string;
  }

  static updateState({ content, updateModel = false, sendToServer = true, language }: UpdateState) {
  ...
  static async convertToYaml() {
    try {
      ...
      yamlContent &&
        this.updateState({ content: yamlContent, updateModel: true, language: 'yaml' });
    } catch (err) {
      console.error(err);
      throw err;
    }
  ...
  static async convertToJSON() {
    try {
      ...
      jsonContent &&
        this.updateState({ content: jsonContent, updateModel: true, language: 'json' });
    } catch (err) {
      console.error(err);
      throw err;
    }
